...
def test_conflict_downgraded_to_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="app.telegram.handlers"):
        asyncio.run(handlers._error_handler(None, _Ctx(Conflict("terminated by other getUpdates"))))
...
def test_network_error_first_logs_detail(caplog):
    """网络异常首条打详情。"""
    with caplog.at_level(logging.WARNING, logger="app.telegram.handlers"):
        asyncio.run(handlers._error_handler(None, _Ctx(NetworkError("httpx.ConnectError"))))
...
def test_network_error_storm_suppressed_then_summary(caplog, monkeypatch):
    ...
    with caplog.at_level(logging.WARNING, logger="app.telegram.handlers"):
        for _ in range(10):
            asyncio.run(handlers._error_handler(None, _Ctx(NetworkError("httpx.ReadTimeout"))))
    warns = [r for r in caplog.records if "TG 网络异常" in r.message]
    assert len(warns) == 1  # 窗口内只打 1 次详情
    assert handlers._net_warn["NetworkError"][1] == 10  # 计数累计
...
def test_proxy_failure_hint(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="app.telegram.handlers"):
        asyncio.run(
            handlers._error_handler(None, _Ctx(NetworkError("[Errno 111] Connection refused")))
        )
...
def test_other_error_keeps_error_level(caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="app.telegram.handlers"):
        asyncio.run(handlers._error_handler(None, _Ctx(RuntimeError("boom"))))
